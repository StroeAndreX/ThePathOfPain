...
for (var i = 1; i <= string_length(_str2); i++){
    draw_text_transformed(20 - 5 + _sW, floor(i / 8) + 1, string_hash_to_newline(string_char_at(_str2, i)), 1, 1, 0);
    _sW += string_width(string_hash_to_newline(string_char_at(_str2, i)));
}
...
var _sW = 0;
for (var i = 1; i <= string_length(_str2); i++){
    draw_text_transformed(20 + _sW, floor(i / 8) + 1, string_hash_to_newline(string_char_at(_str2, i)), 1, 1, 0);
    _sW += string_width(string_hash_to_newline(string_char_at(_str2, i)));
}
