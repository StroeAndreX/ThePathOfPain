/// @description  shockwave_draw_inverted();

/// @description        Draw the shockwave effect, inverting the alpha and texture "push"
    ...
    if wave_view != -1
       {
        ...
        var ww = __view_get( e__VW.WView, 0 );
        var hh = __view_get( e__VW.HView, 0 );
...
repeat (wave_segment + 1)
    ...
    _xx = _wave_tex_x * (_xx - _vx);        // Get the x texture position
    _yy = _wave_tex_y * (_yy - _vy);        // Get the y texture position
    // Add texture coordinates to the vertices
    vertex_texcoord(wave_vb_outside, _xx, _yy);
    vertex_texcoord(wave_vb_inside, _xx, _yy);
    ...
    vertex_position(wave_vb_outside, _xx, _yy);
    _xx = _xx - _vx + (_cosx *  wave_push);
    _yy = _yy - _vy + (_siny * wave_push);
